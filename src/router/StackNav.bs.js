// Generated by BUCKLESCRIPT VERSION 4.0.18, PLEASE EDIT WITH CARE
'use strict';

var ReasonReact = require("reason-react/src/ReasonReact.js");
var Home$Gitphone = require("../shared/screens/Home.bs.js");
var CommitList$Gitphone = require("../shared/screens/CommitList.bs.js");
var StackNavigator$BsReactNavigation = require("bs-react-navigation/src/StackNavigator.bs.js");

function getScreen(route, navigation) {
  if (route) {
    return /* tuple */[
            ReasonReact.element(undefined, undefined, CommitList$Gitphone.make(navigation, route[0], route[1], /* array */[])),
            {
              title: "CommitList"
            }
          ];
  } else {
    return /* tuple */[
            ReasonReact.element(undefined, undefined, Home$Gitphone.make(navigation, /* array */[])),
            {
              title: "Home"
            }
          ];
  }
}

var Stack = StackNavigator$BsReactNavigation.Create(/* module */[
      /* initialRoute : Home */0,
      /* getScreen */getScreen
    ]);

var render = Stack[/* render */11];

exports.Stack = Stack;
exports.render = render;
/* Stack Not a pure module */
